    ...
    for script in re.split(PARAMETER_SPLITTING_REGEX, conf.preprocess):
        ...
        try:
            ...
            if not os.path.exists(script):
                errMsg = f"preprocess script '{script}' does not exist"
                raise SqlmapFilePathException(errMsg)

            elif not script.endswith(".py"):
                errMsg = f"preprocess script '{script}' should have an extension '.py'"
                raise SqlmapSyntaxException(errMsg)
        except UnicodeDecodeError:
        ...
        infoMsg = f"loading preprocess module '{filename[:-3]}'"
        logger.info(infoMsg)

        if not os.path.exists(os.path.join(dirname, "__init__.py")):
            errMsg = "make sure that there is an empty file '__init__.py' "
            errMsg += f"inside of preprocess scripts directory '{dirname}'"
            raise SqlmapGenericException(errMsg)
        ...
        try:
            ...
            raise SqlmapSyntaxException(
                f"cannot import preprocess module '{getUnicode(filename[:-3])}' ({getSafeExString(ex)})"
            )
        ...
        if not found:
            ...
            errMsg += f"in preprocess script '{script}'"
            raise SqlmapGenericException(errMsg)
        else:
            try:
                ...
                errMsg += f"(Note: find template script at '{filename}')"
                raise SqlmapGenericException(errMsg)
